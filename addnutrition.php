...
if(isset($_POST["Submit"])){
    ...
    if(!empty($nutrition_name) && !empty($description) && isset($_FILES["video"]["tmp_name"]) && isset($_FILES["video"]["name"])) {
        ...
        $insert = mysqli_prepare($conn, "INSERT INTO nutritions (nutrition_name, description, video) VALUES(?, ?, ?)");
        mysqli_stmt_bind_param($insert, "sss", $nutrition_name, $description, $video_up);

        if(mysqli_stmt_execute($insert) && move_uploaded_file($video_location, 'video/'.$video_name)) {
            echo "<script>alert('Nutrition téléchargé avec succès.');</script>";
        } else {
            echo "<script>alert('Un problème s’est produit pour que les Nutrition n’aient pas été téléchargés.');</script>";
        }
                ...
                <div class="col-xl-12 col-xxl-12 col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title">Ajouter Nutrition</h5>
                        </div>
                        <form action="addnutrition.php" method="post" enctype="multipart/form-data" class="p-3">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-lg-6 col-md-6 col-sm-12">
                                        <div class="mb-3">
                                            <label for="nutrition_name" class="form-label">Nutrition Name</label>
                                            <input type="text" class="form-control" id="nutrition_name" name="nutrition_name" required>
                                        </div>
                                    </div>
